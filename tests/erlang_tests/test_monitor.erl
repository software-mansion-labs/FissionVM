...
test_monitor_alias() ->
    P = spawn_opt(fun() -> echo_loop() end, []),
    Mon = monitor(process, P, [{alias, demonitor}]),
    P ! {m1, Mon},
    m1 = recv_one(),
    demonitor(Mon),
    P ! {m2, Mon},
...
test_monitor_alias_explicit_unalias() ->
    P = spawn_opt(fun() -> echo_loop() end, []),
    Mon = monitor(process, P, [{alias, explicit_unalias}]),
    P ! {m1, Mon},
    m1 = recv_one(),
    demonitor(Mon),
    P ! {m2, Mon},
