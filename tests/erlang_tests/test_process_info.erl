...
with_other_pid(Fun) ->
    {Pid, Ref} = spawn_opt(fun() -> receive quit -> ok end end, [monitor]),
    Fun(Pid),
    Pid ! quit,
    normal = receive {'DOWN', Ref, process, Pid, Reason} -> Reason end.

with_dead_pid(Fun) ->
    {DeadPid, Ref} = spawn_opt(fun() -> ok end, [monitor]),
    normal = receive {'DOWN', Ref, process, DeadPid, Reason} -> Reason end,
    Fun(DeadPid).
