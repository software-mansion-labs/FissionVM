...
{
    struct PopcornEtsHashTable *htable = malloc(sizeof(struct PopcornEtsHashTable));
...
#ifdef TRACE_ENABLED
static void print_info(struct PopcornEtsHashTable *hash_table)
{
    fprintf(stderr, "============\n");
    for (size_t i = 0; i < hash_table->capacity; ++i) {
        size_t len = 0;
...
PopcornEtsHashtableErrorCode popcorn_ets_hashtable_insert(struct PopcornEtsHashTable *hash_table, term key, term entry, PopcornEtsHashtableOptions opts, Heap *entry_heap, GlobalContext *global)
{
    ...
    while (node) {
        if (term_compare(key, node->key, TermCompareExact, global) == TermEquals) {
            if (opts & PopcornEtsHashtableAllowOverwrite) {
                node->key = key;
                node->entry = entry;
                memory_destroy_heap(node->heap, global);
                node->heap = entry_heap;
                return PopcornEtsHashtableOk;
            } else {
                return PopcornEtsHashtableKeyAlreadyExists;
            }
        }
        last_node = node;
        node = node->next;
    ...
    if (IS_NULL_PTR(new_node)) {
        return PopcornEtsHashtableError;
    }
