...
static bool get_binary_scope_slice(term binary, term options, BinaryPosLen *scope_slice, Context *ctx)
{
...
static term nif_binary_match(Context *ctx, int argc, term argv[])
{
    ...
    if (UNLIKELY(!get_binary_scope_slice(binary_term, options_term, &scope_slice, ctx))) {
        RAISE_ERROR(BADARG_ATOM);
    }
