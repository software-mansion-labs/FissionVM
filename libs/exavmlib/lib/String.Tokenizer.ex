defmodule String.Tokenizer do
  ...
  {letter_uptitlecase, start, continue, _} =
    Path.join(__DIR__, "tokenizer/UnicodeData.txt")
    |> File.read!()
    |> String.split(["\r\n", "\n"], trim: true)
  ...
  {start, continue, patterns} =
    Path.join(__DIR__, "tokenizer/PropList.txt")
    |> File.read!()
    |> String.split(["\r\n", "\n"], trim: true)
  ...
  restricted =
    Path.join(__DIR__, "tokenizer/IdentifierType.txt")
    |> File.read!()
    |> String.split(["\r\n", "\n"], trim: true)
  ...
  script_aliases =
    Path.join(__DIR__, "tokenizer/PropertyValueAliases.txt")
    |> File.read!()
    |> String.split(["\r\n", "\n"], trim: true)
    |> Enum.flat_map(fn line ->
  ...
  scripts = codepoints_to_scriptset.("tokenizer/Scripts.txt", %{})
  script_extensions = codepoints_to_scriptset.("tokenizer/ScriptExtensions.txt", script_aliases)
