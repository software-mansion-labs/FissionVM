...
-include("../../include/erl_compile.hrl").
...
internal_fun(Input, Opts) ->
    fun() ->
        try
            ...
            Class:Reason:Stk ->
                erlang:display({Class, Reason, Stk}),
...
select_passes([{_, Test, Fun} = P | Ps], Opts) when
    is_function(Test),
    is_function(Fun)
->
    [P | select_passes(Ps, Opts)];
...
make_ssa_check_pass(PassFlag) ->
    F = fun(Code, St) ->
        case beam_ssa_check:module(Code, PassFlag) of
            ok -> {ok, Code, St};
            {error, Errors} -> {error, St#compile{errors = St#compile.errors ++ Errors}}
        end
    end,
        ...
        {ok, {Features, ResWordFun}} ->
            ...
            case R of
                ...
                {ok, Forms0, Extra} ->
                    ...
                    Forms =
                        case with_columns(Opts ++ compile_options(Forms0)) of
                            true ->
                                Forms0;
                            false ->
        ...
        true ->
            ...
            try Run({Name, Fun}, StrippedCode, St) of
                ...
                Class:Reason:Stk ->
                    Es = [
                        {St#compile.ifile, [
                            {none, compile, {parse_transform, T, {Class, Reason, Stk}}}
                        ]}
                    ],
                    {error, St#compile{errors = St#compile.errors ++ Es}}
            end;
...
strip_columns(Code) ->
    ...
    [
        case Form of
            {eof, {Line, _Col}} ->
                {eof, Line};
            {ErrorOrWarning, {{Line, _Col}, Module, Reason}} when
...
foldl_core_transforms([T | Ts], Code0, St) ->
    ...
    try Run({Name, Fun}, Code0, St) of
        ...
        Class:Reason:Stk ->
            Es = [{St#compile.ifile, [{none, compile, {core_transform, T, {Class, Reason, Stk}}}]}],
            {error, St#compile{errors = St#compile.errors ++ Es}}
    end;
        ...
        ok ->
            case file:rename(Tfile, Ofile) of
                ok ->
                    {ok, none, St};
                {error, RenameError} ->
                    Es = [{Ofile, [{none, ?MODULE, {rename, Tfile, Ofile, RenameError}}]}],
                    _ = file:delete(Tfile),
                    {error, St#compile{errors = St#compile.errors ++ Es}}
            end;
        {error, Error} ->
...
report_errors(#compile{options = Opts, errors = Errors}) ->
    case member(report_errors, Opts) of
        true ->
            foreach(
                fun
                    ({{F, _L}, Eds}) -> sys_messages:list_errors(F, Eds, Opts);
                    ({F, Eds}) -> sys_messages:list_errors(F, Eds, Opts)
                end,
                Errors
            );
        false ->
...
report_warnings(#compile{options = Opts, warnings = Ws0}) ->
    ...
    case member(report_warnings, Opts) orelse ReportWerror of
        true ->
            Ws1 = flatmap(
                fun
                    ({{F, _L}, Eds}) -> sys_messages:format_messages(F, P, Eds, Opts);
                    ({F, Eds}) -> sys_messages:format_messages(F, P, Eds, Opts)
                end,
                Ws0
            ),
            Ws = lists:sort(Ws1),
