...
expr(
    #c_try{
        ...
        vars = [#c_var{name = X}],
        body = #c_var{name = X},
        handler = #c_literal{val = false} = False
    } = Try,
    _,
    Sub
) ->
...
is_safe_simple(#c_tuple{es = Es}) ->
    is_safe_simple_list(Es);
is_safe_simple(#c_literal{}) ->
    true;
is_safe_simple(#c_call{
    module = #c_literal{val = erlang},
...
bin_un_utf_1(
    [#c_bitstr{val = #c_literal{}, type = #c_literal{val = utf8}} = H | T],
    Anno
) ->
    bin_un_utf_eval(H, Anno) ++ bin_un_utf_1(T, Anno);
bin_un_utf_1(
    [#c_bitstr{val = #c_literal{}, type = #c_literal{val = utf16}} = H | T],
    Anno
) ->
    bin_un_utf_eval(H, Anno) ++ bin_un_utf_1(T, Anno);
bin_un_utf_1(
    [#c_bitstr{val = #c_literal{}, type = #c_literal{val = utf32}} = H | T],
    Anno
) ->
    bin_un_utf_eval(H, Anno) ++ bin_un_utf_1(T, Anno);
...
fold_call(Call, _M, _N, _Args, _Sub) ->
    Call.
...
fold_non_lit_args(Call, lists, append, [Arg1, Arg2], _) ->
    eval_append(Call, Arg1, Arg2);
fold_non_lit_args(Call, _, _, _, _) ->
    Call.
...
eval_append(Call, X, Y) ->
    %Rebuild call arguments.
    Call#c_call{args = [X, Y]}.
...
get_fixed_args(#c_literal{val = MoreArgs0}, Args) when
    length(MoreArgs0) >= 0
->
...
let_substs(Vs0, As0, Sub0) ->
    ...
    {Vs2, As1, foldl(fun({V, S}, Sub) -> sub_set_name(V, S, Sub) end, Sub2, Ss)}.

let_substs_1(Vs, #c_values{es = As}, Sub) ->
    let_subst_list(Vs, As, Sub);
let_substs_1([V], A, Sub) ->
...
map_pair_pattern(
    #c_map_pair{op = #c_literal{val = exact}, key = K0, val = V0} = Pair, {Isub, Osub0}
) ->
    K = expr(K0, Isub),
...
sub_add_scope(Vs, #sub{s = Scope0} = Sub) ->
    Scope = foldl(
        fun(V, S) when is_integer(V); is_atom(V) ->
...
propagate_compiler_generated(From, To) ->
    case
        is_compiler_generated(From) andalso
            not is_compiler_generated(To)
    of
        true ->
...
warn_no_clause_match(CaseOrig, CaseOpt) ->
    ...
    case
        any(fun(C) -> not is_compiler_generated(C) end, OrigCs) andalso
            all(fun is_compiler_generated/1, OptCs)
    of
        true ->
...
opt_bool_case_in_let_1(
    [#c_var{name = V}],
    Arg,
    #c_case{arg = #c_var{name = V}} = Case0,
    Let,
    Sub
) ->
...
opt_bool_case_in_let_1(_, _, _, Let, _) ->
    Let.
...
do_opt_fun_call(
    ...
    FunDef,
    #c_let{
        arg = #c_apply{op = #c_var{name = V}, args = CallArgs},
        body = Rest
    } = Let
) ->
...
opt_build_stacktrace(
    #c_let{
        vars = [#c_var{name = Cooked}],
        arg = #c_primop{
...
matches_data([#c_var{name = V}], #c_case{
    arg = #c_var{name = V},
...
move_case_into_arg(
    #c_case{
        arg = #c_seq{arg = OuterArg, body = InnerArg} = Outer,
        clauses = InnerClauses
    } = Inner,
    _Sub
) ->
...
update_let_types_1([], [], Sub) ->
    Sub.

update_types(V, #c_tuple{} = P, #sub{t = Tdb} = Sub) ->
    Sub#sub{t = Tdb#{V => P}};
update_types(_, _, Sub) ->
    Sub.
...
format_error({failed, {eval_failure, Call, Reason}}) ->
    flatten(
        io_lib:format(
            "~ts will fail with a '~p' exception",
...
format_error({failed, {embedded_unit, Unit, Size}}) ->
    M = io_lib:format(
        "binary construction will fail with a 'badarg' exception "
        "(size ~p cannot be evenly divided by unit ~p)",
        [Size, Unit]
    ),
    flatten(M);
...
format_error({nomatch, {shadow, Line, {Name, Arity}}}) ->
    M = io_lib:format(
        "this clause for ~ts/~B cannot match because a previous "
        "clause at line ~p always matches",
        [Name, Arity, Line]
    ),
    flatten(M);
format_error({nomatch, {shadow, Line}}) ->
    M = io_lib:format(
        "this clause cannot match because a previous clause at line ~p "
        "always matches",
        [Line]
    ),
    flatten(M);
...
format_error({nomatch, {bit_syntax_truncated, Signess, Val, Sz}}) ->
    ...
    F =
        "this clause cannot match because the value ~P"
...
format_error({nomatch, {bit_syntax_unsigned, Val}}) ->
    F =
        "this clause cannot match because the negative value ~P"
...
format_error({nomatch, {bit_syntax_type, Val, Type}}) ->
    F =
        "this clause cannot match because '~P' is not of the"
        " expected type '~p'",
