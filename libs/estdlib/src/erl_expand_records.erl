...
-record(exprec, {
    ...
    checked_ra = [],
    % Cached value of compile flag 'dialyzer'
    dialyzer = false
}).
...
forms([{attribute, _, record, {Name, Defs}} = Attr | Fs], St0) ->
    ...
    St = St0#exprec{
        records = maps:put(Name, NDefs, St0#exprec.records),
...
expr({call, Anno, {remote, _, {atom, _, erlang}, {atom, _, is_record}}, [A, {atom, _, Name}]}, St) ->
    record_test(Anno, A, Name, St);
expr({call, Anno, {tuple, _, [{atom, _, erlang}, {atom, _, is_record}]}, [A, {atom, _, Name}]}, St) ->
    record_test(Anno, A, Name, St);
...
expr({op, Anno, Op, A0}, St0) ->
    {A, St1} = expr(A0, St0),
    {{op, Anno, Op, A}, St1};
expr({op, Anno, Op, L0, R0}, St0) when
    Op =:= 'and';
    Op =:= 'or'
->
...
strict_get_record_field(Anno, R, {atom, _, F} = Index, Name, St0) ->
    case is_in_guard() of
        ...
        true ->
            ...
            {
                {call, Anno, {remote, Anno, {atom, Anno, erlang}, {atom, Anno, element}}, [I, ExpR]},
                St2
            }
    end.
...
sloppy_get_record_field(Anno, R, Index, Name, St) ->
    Fs = record_fields(Name, Anno, St),
    I = index_expr(Anno, Index, Name, Fs),
    expr({call, Anno, {remote, Anno, {atom, Anno, erlang}, {atom, Anno, element}}, [I, R]}, St).
...
pattern_fields(Fs, Ms) ->
    ...
    map(
        fun({record_field, Anno, {atom, _, F}, _}) ->
...
record_setel(R, Name, Fs, Us0) ->
    Us1 = foldl(
        fun({record_field, Anno, Field, Val}, Acc) ->
...
opt_rec_vars_2(
    {call, _, {atom, _, is_record}, [{var, _, V}, {atom, _, Tag}, {integer, _, Sz}]}, Rs
) ->
    orddict:store(V, {Tag, Sz}, Rs);
opt_rec_vars_2(_, Rs) ->
    Rs.
...
opt_remove_2(
    {call, Anno, {atom, _, is_record}, [{var, _, V}, {atom, _, Tag}, {integer, _, Sz}]} = A, Rs
) ->
    case orddict:find(V, Rs) of
        {ok, {remove, Tag, Sz}} ->
            {atom, Anno, true};
