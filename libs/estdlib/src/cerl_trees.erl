    ...
    case type(T) of
        ...
        'receive' ->
            fold(
                ...
                fold(
                    F,
                    fold_list(F, S, receive_clauses(T)),
                    receive_timeout(T)
                ),
                receive_action(T)
            );
...
-spec mapfold(
    fun((cerl:cerl(), term()) -> {cerl:cerl(), term()} | skip),
    fun((cerl:cerl(), term()) -> {cerl:cerl(), term()}),
    term(),
    cerl:cerl()
) -> {cerl:cerl(), term()}.
