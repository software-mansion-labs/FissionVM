...
%Not used in patterns.
-record(ibinary, {anno = #a{}, segments}).
...
%Variable counter
-record(core, {
    vcount = 0 :: non_neg_integer(),
    %Function counter
    fcount = 0 :: non_neg_integer(),
    %Goto counter
    gcount = 0 :: non_neg_integer(),
    %Current function.
    function = {none, 0} :: fa(),
    %In guard or not.
    in_guard = false :: boolean(),
    %Result wanted or not.
    wanted = true :: boolean(),
    %Options.
    opts = [] :: [compile:option()],
    %Help dialyzer or not.
...
module(Forms0, Opts) ->
    ...
    Exp =
        case member(export_all, Opts) of
...
function({function, _, Name, Arity, Cs0}, Module, Opts) when
    is_integer(Arity), 0 =< Arity, Arity =< 255
->
...
gexpr_not(A, Bools0, St0, Line) ->
    ...
    case Ae0 of
        #icall{
            anno = #a{anno = [v3_core, compiler_generated]},
            module = #c_literal{val = erlang},
            name = #c_literal{val = '=:='},
            args = [E, #c_literal{val = true}]
        } = EqCall ->
            %%
...
gexpr_test(E0, Bools0, St0) ->
    ...
    case E1 of
        #icall{
            ...
            {icall_eq_true(New), Eps0 ++ [#iset{anno = Anno, var = New, arg = E1}], Bools0, St2};
        #icall{
            anno = Anno, module = #c_literal{val = erlang}, name = #c_literal{val = N}, args = As
        } ->
            ...
            Ar = length(As),
            case
                erl_internal:new_type_test(N, Ar) orelse
            ...
            of
                ...
                false ->
                    ...
                    {
                        icall_eq_true(New),
                        Eps0 ++ [#iset{anno = Anno, var = New, arg = E1}],
                        Bools,
                        St2
                    }
            end;
        _ ->
            ...
            case is_simple(E1) of
                ...
                false ->
                    ...
                    {
                        icall_eq_true(New),
                        Eps0 ++ [#iset{anno = ACompGen, var = New, arg = E1}],
                        Bools,
                        St2
                    }
            end
...
maybe_match_exprs([{maybe_match, L, P0, E0} | Es0], Fail, St0) ->
    ...
    Fc = #iclause{
        anno = #a{anno = [dialyzer_ignore, compiler_generated | Lanno]},
        pats = [Fpat],
        guard = [],
...
expr({var, L, V}, St) ->
    {#c_var{anno = lineno_anno(L, St), name = V}, [], St};
expr({char, L, C}, St) ->
    {#c_literal{anno = full_anno(L, St), val = C}, [], St};
expr({integer, L, I}, St) ->
    {#c_literal{anno = full_anno(L, St), val = I}, [], St};
expr({float, L, F}, St) ->
    {#c_literal{anno = full_anno(L, St), val = F}, [], St};
expr({atom, L, A}, St) ->
    {#c_literal{anno = full_anno(L, St), val = A}, [], St};
expr({nil, L}, St) ->
    {#c_literal{anno = full_anno(L, St), val = []}, [], St};
expr({string, L, S}, St) ->
    {#c_literal{anno = full_anno(L, St), val = S}, [], St};
...
expr({'try', L, Es0, Cs0, Ecs, []}, St0) ->
    ...
    {
        #itry{
            anno = #a{anno = lineno_anno(L, St5)},
            args = Es1,
            vars = [V],
            body = [#icase{anno = #a{anno = Lanno}, args = [V], clauses = Cs1, fc = Fc}],
            evars = Evs,
            handler = Hs
        },
        [],
...
expr({call, L, {remote, _, M0, F0}, As0}, St0) ->
    ...
    case {M1, F1, As1} of
        {#c_literal{val = erlang}, #c_literal{val = error}, [
        ...
        {#c_literal{val = erlang}, #c_literal{val = load_nif}, [_, _]} ->
            {#icall{anno = #a{anno = Anno}, module = M1, name = F1, args = As1}, Aps, St1#core{
                load_nif = true
            }};
        {_, _, _} ->
...
expr({op, L, Op, A0}, St0) ->
    ...
    %Must have an #a{}
    {
        #icall{
            anno = #a{anno = LineAnno},
            module = #c_literal{anno = LineAnno, val = erlang},
            name = #c_literal{anno = LineAnno, val = Op},
            args = [A1]
        },
        Aps,
        St1
...
expr({ssa_check_when, L, WantedResult, Args, Tag, Clauses}, St) ->
    {
        #c_opaque{
            anno = full_anno(L, St), val = {ssa_check_when, WantedResult, Tag, Args, Clauses}
        },
        [],
        St
    }.
...
try_exception(Ecs0, St0) ->
    ...
    Ec = #iclause{
        anno = #a{anno = [compiler_generated | LA]},
        pats = [c_tuple(Evs)],
        guard = [#c_literal{val = true}],
        %Must have an #a{}
        body = [
...
bitstr({bin_element, {sl, _, Line}, {string, _, S}, Sz0, Ts}, St0) ->
    ...
    Es = [
        Bitstr#c_bitstr{val = #c_literal{anno = full_anno(Line, St1), val = C}}
     || C <- S
    ],
    {Es, Eps, St1};
...
lc_tq(
    ...
    SkipClause = make_clause(
        [skip_clause, compiler_generated | LA],
        SkipPat,
        [],
        [],
        [Nc]
    ...
    {
        #iletrec{
            anno = GAnno#a{anno = [list_comprehension | GA]},
            defs = [{{Name, 1}, Fun}],
        ...
        PrePre ++
            [
                #iset{
                    var = BinVar,
                    arg = #iprimop{
                        anno = #a{anno = lineno_anno(Line, St)},
...
bc_tq1(
    ...
    SkipClause = make_clause(
        [compiler_generated, skip_clause | LA],
        SkipPat,
        [IgnoreVar],
        [],
        [Nc]
    ...
    {
        #iletrec{
            anno = LAnno#a{anno = [list_comprehension, no_inline | LA]},
            defs = [{{Name, 2}, Fun}],
...
filter_tq(
    Line,
    E,
    #ifilter{anno = #a{anno = LA} = LAnno, arg = {Pre, Arg}},
    Mc,
    St0,
    Qs,
    TqFun
    ...
    {
        #icase{
            anno = LAnno#a{anno = [list_comprehension | LA]},
            args = [Arg],
            clauses = [
                #iclause{
                    anno = LAnno,
                    pats = [#c_literal{val = true}],
                    guard = [],
                    body = Lps ++ [Lc]
                },
                #iclause{
                    anno = LAnno#a{anno = [compiler_generated | LA]},
                    pats = [#c_literal{val = false}],
                    guard = [],
                    body = [Mc]
                }
            ],
...
filter_tq(
    Line,
    E,
    #ifilter{anno = #a{anno = LA} = LAnno, arg = Guard},
    Mc,
    St0,
    Qs,
    TqFun
...
generator(Line, {generate, Lg, P0, E}, Gs, St0) ->
    ...
    Gen = #igen{
        ...
        skip_pat = SkipPat,
        tail = Tail,
        tail_pat = #c_literal{anno = LA, val = []},
        arg = {Pre, Ce}
    },
    {Gen, St4};
...
generator(Line, {m_generate, Lg, {map_field_exact, _, K0, V0}, E}, Gs, St0) ->
    ...
    Gen = #igen{
        ...
        skip_pat = SkipPat,
        tail = IterVar,
        tail_pat = #c_literal{anno = LA, val = none},
        refill = Refill,
...
%These are novars too
force_novars(#ifun{} = Fun, St) -> {Fun, [], St};
force_novars(#ibinary{} = Bin, St) -> {Bin, [], St};
force_novars(#c_map{} = Bin, St) -> {Bin, [], St};
force_novars(Ce, St) -> force_safe(Ce, St).
...
%Fun. Not safe.
is_safe(#c_var{name = {_, _}}) -> false;
%Ordinary variable.
is_safe(#c_var{name = _}) -> true;
...
pattern({var, L, V}, St) ->
    {#c_var{anno = lineno_anno(L, St), name = V}, St};
pattern({char, L, C}, St) ->
    {#c_literal{anno = lineno_anno(L, St), val = C}, St};
pattern({integer, L, I}, St) ->
    {#c_literal{anno = lineno_anno(L, St), val = I}, St};
pattern({float, L, F}, St) ->
    {#c_literal{anno = lineno_anno(L, St), val = F}, St};
pattern({atom, L, A}, St) ->
    {#c_literal{anno = lineno_anno(L, St), val = A}, St};
pattern({string, L, S}, St) ->
    {#c_literal{anno = lineno_anno(L, St), val = S}, St};
pattern({nil, L}, St) ->
    {#c_literal{anno = lineno_anno(L, St), val = []}, St};
...
pat_bin_expand_strings(Es0, #core{dialyzer = Dialyzer}) ->
    foldr(
        fun
            ({bin_element, Line, {string, _, [_ | _] = S}, default, default}, Es1) when
                not Dialyzer
            ->
...
pat_alias(#c_var{name = V1} = P, #c_var{name = V1}) ->
    P;
pat_alias(
    #c_var{name = V1} = Var,
...
pat_alias(#c_var{} = P1, P2) ->
    #c_alias{var = P1, pat = P2};
pat_alias(#c_alias{var = #c_var{name = V1}} = Alias, #c_var{name = V1}) ->
    Alias;
pat_alias(
    #c_alias{var = #c_var{name = V1} = Var1, pat = P1},
...
do_uclause(#iclause{anno = A0, pats = Ps0, guard = G0, body = B0}, Ks0, St0) ->
    ...
    Anno = A0#a.anno,
    {Pg, A} =
        case member(skip_clause, Anno) of
            true ->
...
uexprs_iset([#iset{anno = A0, var = V, arg = Arg0} = Le0 | Les0], New0, Ks0, St0) ->
    case uexpr_need_known(Arg0) of
        ...
        false ->
            ...
            A = A0#a{
                us = del_element(V#c_var.name, Us),
...
uexpr(#iset{anno = A, var = V, arg = A0}, Ks, St0) ->
    {A1, St1} = uexpr(A0, Ks, St0),
    {
        #iset{
            anno = A#a{
                us = del_element(V#c_var.name, (get_anno(A1))#a.us),
...
uexpr(#icase{anno = #a{anno = Anno} = A, args = As0, clauses = Cs0, fc = Fc0}, Ks, St0) ->
    ...
    New =
        case member(list_comprehension, Anno) of
            true -> [];
...
uexpr(#ireceive1{anno = A, clauses = Cs0}, Ks, St0) ->
    ...
    {
        #ireceive1{
            anno = A#a{us = used_in_any(Cs1), ns = new_in_all(Cs1)},
            clauses = Cs1
        },
        St1
    };
...
uexpr(#iprotect{anno = A, body = Es0}, Ks, St0) ->
    ...
    {#iprotect{anno = A#a{us = Used}, body = Es1}, St1};
...
upattern(#c_var{name = V} = Var, Ks, St0) ->
    case is_element(V, known_get(Ks)) of
        true ->
            ...
            Test = #icall{
                anno = #a{anno = LA, us = add_element(N, [V])},
                module = #c_literal{val = erlang},
                name = #c_literal{val = '=:='},
                args = [New, Var]
            },
...
upat_element(#ibitstr{val = H0, size = Sz0} = Seg, Ks, Bs0, St0) ->
    ...
    case Sz0 of
        ...
        Expr when is_list(Expr) ->
            Sz1 =
                [
                    #iset{var = #c_var{name = Old}, arg = #c_var{name = New}}
                 || {Old, New} <- Bs0
                ] ++ Expr,
            {Sz2, _, St2} = uexprs(Sz1, Ks, St1),
            Us = used_in_expr(Sz2),
...
used_in_any(Les) ->
    foldl(
        fun(Le, Ns) -> union((get_anno(Le))#a.us, Ns) end,
        [],
        Les
    ).
...
new_in_any(Les) ->
    foldl(
        fun(Le, Ns) -> union((get_anno(Le))#a.ns, Ns) end,
        [],
        Les
    ).
...
new_in_all([Le | Les]) ->
    foldl(
        fun(L, Ns) -> intersection((get_anno(L))#a.ns, Ns) end,
        (get_anno(Le))#a.ns,
        Les
    );
new_in_all([]) ->
...
cbody(B0, Nifs, St0) ->
    {B1, _, _, St1} = cexpr(B0, [], St0),
    B2 =
        case sets:is_element(St1#core.function, Nifs) of
            true ->
                #c_fun{body = Body0} = B1,
                Body1 = #c_seq{
                    arg = #c_primop{
                        name = #c_literal{val = nif_start},
                        args = []
                    },
                    body = Body0
                },
...
cexprs([#iset{anno = #a{anno = A}, var = V, arg = A0} | Les], As0, St0) ->
    ...
    {#c_let{anno = A, vars = [V | make_vars(Es)], arg = A1, body = Ces}, union(Us, As1), St2};
cexprs([Le | Les], As0, St0) ->
    ...
    if
        ...
        true ->
            {R, St3} = new_var(St2),
            {#c_let{vars = [R | make_vars(Es)], arg = Ce, body = Ces}, union(Us, As1), St3}
    end.
...
cexpr(#iletrec{anno = A, defs = Fs0, body = B0}, As, St0) ->
    {Fs1, {_, St1}} = mapfoldl(
        fun({{_Name, _Arity} = NA, F0}, {Used, S0}) ->
            {F1, [], Us, S1} = cexpr(F0, [], S0),
            {{#c_var{name = NA}, F1}, {union(Us, Used), S1}}
        end,
        {[], St0},
        Fs0
    ),
...
cexpr(#icase{anno = A, args = Largs, clauses = Lcs, fc = Lfc}, As, St0) ->
    ...
    {Cfc, St4} = c_add_dummy_export(Cfc0, Exp, St3),
    {
        #c_case{
            anno = A#a.anno,
            arg = core_lib:make_values(Cargs),
            clauses = Ccs ++ [Cfc]
        },
        Exp,
        A#a.us,
...
cexpr(#ireceive1{anno = A, clauses = Lcs}, As, St0) ->
    ...
    Action = core_lib:make_values(lists:duplicate(1 + length(Exp), True)),
    {
        #c_receive{
            anno = A#a.anno,
            clauses = Ccs,
            timeout = #c_literal{val = infinity},
            action = Action
        },
        Exp,
        A#a.us,
...
cexpr(#ifun{name = unnamed} = Fun, As, St0) ->
    cfun(Fun, As, St0);
cexpr(
    #ifun{anno = #a{us = Us0} = A0, name = {named, Name}, fc = #iclause{pats = Ps}} = Fun0,
    As,
    St0
) ->
...
lexpr(#c_receive{anno = RecvAnno, clauses = Cs0, timeout = Timeout0, action = Action}, St0) ->
    ...
    Cs1 = rewrite_cs(Cs0),
    RecvNext = #c_seq{
        arg = primop(recv_next),
        body = ApplyLoop
    },
    RecvNextC = #c_clause{
        anno = [compiler_generated, dialyzer_ignore],
        pats = [#c_var{name = 'Other'}],
        guard = True,
        body = RecvNext
    },
    Cs = Cs1 ++ [RecvNextC],
    ...
    AfterCs = [
        #c_clause{pats = [True], guard = True, body = Action},
        #c_clause{
            anno = [dialyzer_ignore],
            pats = [False],
            guard = True,
            body = ApplyLoop
        }
...
split_reconstruct(
    ...
    #c_clause{anno = Anno} = C0,
    CaseAnno,
    St0
) ->
    {InnerCase, St1} = split_reconstruct(
        SplitArgs,
        [Pat],
        Nested,
        C0,
    ...
    {Evars, St1} = new_vars(3, St0),
    {
        fun(Body) ->
            Try = #c_try{
                ...
                body = SizeVar,
                evars = Evars,
                handler = #c_literal{val = bad_size}
            },
...
is_simple(#c_cons{hd = H, tl = T}) -> is_simple(H) andalso is_simple(T);
is_simple(#c_tuple{es = Es}) -> is_simple_list(Es);
is_simple(#c_map{es = Es}) -> is_simple_list(Es);
is_simple(#c_map_pair{key = K, val = V}) -> is_simple(K) andalso is_simple(V);
is_simple(_) -> false.
