...
-export([
    await_microstate_accounting_modifications/3,
...
-export([
    ...
    system_flag_scheduler_wall_time/1,
...
microstate_accounting(Ref, Threads) ->
    receive
        ...
        {Ref, Res} -> [Res | microstate_accounting(Ref, Threads - 1)]
    end.
...
-spec erts_internal:get_dflags() ->
    {erts_dflags, integer(), integer(), integer(), integer(), integer()}.
get_dflags() ->
    erlang:nif_error(undefined).
...
-spec erts_internal:gather_sched_wall_time_result(Ref) ->
...
sched_wall_time(Ref, N, undefined) ->
    receive
        {Ref, _} -> sched_wall_time(Ref, N - 1, undefined)
    end;
...
ets_info_binary_iter(Tab, Key, Acc) ->
    NewAcc =
        case erts_internal:ets_lookup_binary_info(Tab, Key) of
            [] -> Acc;
            [BI] -> [BI | Acc];
            [_ | _] = BIL -> BIL ++ Acc
        end,
...
-spec dist_spawn_request
    (Node, MFA, Opts, spawn_request) -> Res when
        Node :: node(),
...
-spec list_to_integer(List, Base) ->
    {integer(), list()} | 'big' | 'badarg' | 'no_integer' | 'not_a_list'
when
    List :: [any()],
    Base :: 2..36.
...
dynamic_node_name(false) ->
    case dynamic_node_name() of
        false ->
            ok;
        _ ->
            _ = persistent_term:erase({?MODULE, dynamic_node_name}),
            ok
    end.
