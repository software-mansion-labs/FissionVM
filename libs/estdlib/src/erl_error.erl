...
-export([
    format_exception/6, format_exception/7, format_exception/8,
    format_stacktrace/4, format_stacktrace/5,
    format_call/4, format_call/5,
    format_fun/1, format_fun/2
]).
...
%% -> iolist() | unicode:charlist()  (no \n at end)
format_stacktrace(I, StackTrace, StackFun, FormatFun, Encoding) when
    is_integer(I),
    I >= 1,
    is_function(StackFun, 3),
...
%% -> iolist() | unicode:charlist()  (no \n at end)
format_call(I, ForMForFun, As, FormatFun, Enc) when
    is_integer(I), I >= 1, is_list(As), is_function(FormatFun, 2)
->
    FF = wrap_format_fun_2(FormatFun),
...
is_stacktrace([]) ->
    true;
is_stacktrace([{M, F, A, I} | Fs]) when
    is_atom(M), is_atom(F), is_integer(A), is_list(I)
->
    is_stacktrace(Fs);
is_stacktrace([{M, F, As, I} | Fs]) when
    is_atom(M), is_atom(F), length(As) >= 0, is_list(I)
->
    is_stacktrace(Fs);
is_stacktrace(_) ->
    false.
...
explain_reason({badarity, {Fun, As}}, error, [], _PF, _S, Enc, _CL) when
    is_function(Fun)
->
...
explain_reason({argument_limit, _Fun}, error, [], _PF, _S, _Enc, _CL) ->
    io_lib:fwrite(
        <<
            "limit of number of arguments to interpreted function"
            " exceeded"
        >>,
        []
    );
...
explain_reason(
    {restricted_shell_disallowed, {ForMF, As}},
    exit = Cl,
    [],
    PF,
    S,
...
format_stacktrace1(S0, Stack0, PF, SF, Enc, CL, Reason, ErrorMap) ->
    Stack1 = lists:dropwhile(fun({M, F, A, _}) -> SF(M, F, A) end, lists:reverse(Stack0)),
    S = ["  " | S0],
...
format_stacktrace2(_S, _Stack, _N, _PF, _Enc, _CL = 0, _Reason, _ErrorMap) ->
    [];
format_stacktrace2(S, [{M, F, A, L} | Fs], N, PF, Enc, CL, Reason, ErrorMap) when
    is_integer(A)
->
...
format_stacktrace2(S, [{M, F, As, _Info} | Fs], N, PF, Enc, CL, Reason, ErrorMap) when
    is_list(As)
->
    A = length(As),
