...
-export([
    ...
    range/1,
    field/1,
    relative_product/1, relative_product/2,
    relative_product1/2,
    converse/1,
    image/2,
    inverse_image/2,
...
-export([
    family_to_digraph/1, family_to_digraph/2,
...
% true for ?ANYTYPE...
-define(IS_ATOM_TYPE(T), is_atom(T)).
...
canonical_relation(Sets) when ?IS_SET(Sets) ->
    ...
    case ST of
        ...
        ?SET_OF(Type) -> ?SET(can_rel(?LIST(Sets), []), ?BINREL(Type, ST));
        ?ANYTYPE -> Sets;
        _ -> erlang:error(badarg)
    end.
...
-spec relative_product
    (ListOfBinRels, BinRel1) -> BinRel2 when
        ListOfBinRels :: [BinRel, ...],
        BinRel :: binary_relation(),
...
-spec join(Relation1, I, Relation2, J) -> Relation3 when
    ...
    I :: pos_integer(),
    J :: pos_integer().
join(R1, I1, R2, I2) when
    ?IS_SET(R1), ?IS_SET(R2), is_integer(I1), is_integer(I2)
->
...
partition_family(I, Set) when is_integer(I), ?IS_SET(Set) ->
    ...
    case check_for_sort(Type, I) of
        ...
        false ->
            ?SET(
                fam_partition_n(I, ?LIST(Set)),
                ?BINREL(?REL_TYPE(I, Type), ?SET_OF(Type))
            );
        true ->
            ?SET(
                fam_partition_n(I, keysort(I, ?LIST(Set))),
                ?BINREL(?REL_TYPE(I, Type), ?SET_OF(Type))
            )
    end;
...
make_element(C, ?ANYTYPE, _T0) ->
    make_element(C);
make_element(C, Atom, ?ANYTYPE) when
    ?IS_ATOM_TYPE(Atom),
    not is_list(C),
    not is_tuple(C)
->
...
sympart(S1, [], L1, L12, L2, T) ->
    {?SET(reverse(L1, S1), T), ?SET(reverse(L12), T), ?SET(reverse(L2), T)};
sympart(_, S2, L1, L12, L2, T) ->
    {?SET(reverse(L1), T), ?SET(reverse(L12), T), ?SET(reverse(L2, S2), T)}.
...
sympart1(_, T2, L1, L12, L2, T, H2) ->
    {?SET(reverse(L1), T), ?SET(reverse(L12), T), ?SET(reverse(L2, [H2 | T2]), T)}.
...
sympart2(T1, _, L1, L12, L2, T, H1) ->
    {?SET(reverse(L1, [H1 | T1]), T), ?SET(reverse(L12), T), ?SET(reverse(L2), T)}.
...
%% Inlined.
unify_types(T, T) -> T;
unify_types(Type1, Type2) -> catch unify_types1(Type1, Type2).
